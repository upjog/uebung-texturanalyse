...
image_path = '../images/synthetic.png'                      # Bildimport Linux
#image_path = './images/synthetic.png'                       # Bildimport Windows
image = cv2.imread(image_path, cv2.IMREAD_GRAYSCALE)

out_path = '../out/synthetic/'   # Linux
#out_path = './out/synthetic'     # Windows
...
gamma = 0.5                 # Ellipzität der Gauß-Funktion
psi = 0                     # Phasenverschiebung der Sinusfunktion
sigma = 5.0                 # Standardabweichung 5

# Winkelausrichtung der Sinusfunktion
#orientations = [0, np.pi/4, np.pi/2, 3*np.pi/4]
#orientations = [0, np.pi/6, np.pi/4, np.pi/2, 3*np.pi/4, 5*np.pi/6] # Hier 6 Winkel vorteilhaft
orientations = [0, np.pi/9, 2*np.pi/9, 3*np.pi/9, 4*np.pi/9, 5*np.pi/9, 6*np.pi/9, 7*np.pi/9, 8*np.pi/9] # Hier 9 Winkel noch vorteilhafter !?
...
wavelengths = wavelengths[:]                       # Begrenze auf kleinere Wellenlängen
...
summed_image_bsp = sum_filtered_images[2]
...
M = int(3 * sigma)      # Wähle in Abhängigkeit der Standardabweichung. Hier Faktor 5 gewählt
...
# AAD für ein Summenbild (beispielhaft)
